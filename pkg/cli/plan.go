...
func NewCmdPlan(in io.Reader, out io.Writer, options *installOpts) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := clusterStore.Put(name, store.Cluster{
				Spec:   store.ClusterSpec{},
				Status: store.ClusterStatus{},
			}); err != nil {
				return nil, fmt.Errorf("error committing cluster to database: %v", err)
			}
...
func doPlan(in io.Reader, out io.Writer, planner install.FilePlanner) (*install.Plan, error) {
